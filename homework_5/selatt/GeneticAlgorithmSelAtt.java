 ...
 * individuals in the population (this is to provide flexibility in terms of how
 * a problem can be encoded).
 */
public class GeneticAlgorithmSelAtt<A> {
     ...
     * generation. This method get the better individual of the generation.
     */
    protected List<Individual<A>> nextGeneration(List<Individual<A>> population, FitnessFunction<A> fitnessFn) {
        ...
        Individual<A> worstInd = retrieveWorstIndividual(population, fitnessFn);

        newPopulation.add(new Individual<A>(retrieveBestIndividual(population, fitnessFn).getRepresentation()));
        ...
        do {
            ...
            // add child to new_population
            if (acceptance(child, worstInd, fitnessFn)) {
...
//        }
    ...
    protected boolean acceptance(Individual<A> childOne, Individual<A> worstIndividual, FitnessFunction<A> fitnessFn) {
        double fChildOne = fitnessFn.apply(childOne);
        double fWorstindividual = fitnessFn.apply(worstIndividual);

        return fChildOne > fWorstindividual;
    }
    ...
    // function REPRODUCE(x, y) returns an individual
    // inputs: x, y, parent individuals
    protected Individual<A> reproduceMod(Individual<A> x, Individual<A> y) {
        // n <- LENGTH(x);
        // Note: this is = this.individualLength
        // c <- random number from 1 to n
        int c = randomOffset(individualLength);
    ...
    protected Individual<A> mutateMod(Individual<A> child) {
        int mutateOffset1 = randomOffset(individualLength);
        int mutateOffset2 = randomOffset(individualLength);
        int alphaOffset1 = randomOffset(finiteAlphabet.size());
        int alphaOffset2 = randomOffset(finiteAlphabet.size());

        List<A> mutatedRepresentation = new ArrayList<A>(child.getRepresentation());

        mutatedRepresentation.set(mutateOffset1, finiteAlphabet.get(alphaOffset1));
        mutatedRepresentation.set(mutateOffset2, finiteAlphabet.get(alphaOffset2));
